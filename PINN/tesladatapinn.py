...
class TeslaDatasetPinn(Dataset):
    def __init__(self, pData = 'tesla_driving_temp_data.csv',ID = -1, device = "cuda:0", normalize = 1, data = "train", battery_level = True, diff = "fwd_diff"):
        ...
        for k in range(idxx.shape[0]-1):
          ...
          end_idx = idxx[k+1]
          #print(end_idx)
          #df0["central_diff"].iloc[start_idx:end_idx] = df0['time'].iloc[start_idx:end_idx]-df0['time'].iloc[start_idx]
          xx = df0['rel_time'].iloc[start_idx:end_idx].values
          yy = df0['battery_temperature'].iloc[start_idx:end_idx].values
          #print('xx',xx.reshape(-1))
          #print('yy',yy.reshape(-1))
        ...
        #   if data == "train":
        #      print(df0.shape)
        #      df0 = df0.head(int(len(df0)*(80/100)))
        #      print(df0.shape,'df0')
        #   elif data == "test":
        #     print(df0.shape,'df0')
...
#         df_y = df0[["battery_temperature"]]
        ...
        if battery_level == True:
          #print(rel_time)
          # Extract features and labels
          df_x = df0[["power","speed", "battery_level", "outside_temp", "rel_time"]]
        else:
          # Extract features and labels
          df_x = df0[["power","speed", "outside_temp",  "rel_time"]]
        ...
        ids = df0[["drive_id"]]
        ids = torch.tensor(ids.values).float()
        ...
        if battery_level == True:
            ...
            lb[2]=df[['battery_level']].min()
            ub[2]=df[['battery_level']].max()
            ...
            lb[2]=df[['outside_temp']].min()
            ub[2]=df[['outside_temp']].max()
    ...
    def __getitem__(self, index):
        ...
        return self.x[start_idx:end_idx],self.y[start_idx:end_idx],self.temp[start_idx:end_idx],self.ids[start_idx:end_idx]
